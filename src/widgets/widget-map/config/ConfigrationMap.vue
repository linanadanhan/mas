<template>
  <div class="widget">
    <el-form ref="form" :model="form" label-width="140px">
      <el-form-item prop="areaCode" label="行政区划地图">
        <el-cascader placeholder="请选择展示的行政规划" :change-on-select="true" :options="options" v-model="selectedOptions"
                     @change="handleChange">
        </el-cascader>
      </el-form-item>
    </el-form>
  ...
  export default {
    ...
    methods: {
      handleChange(value) {
        ...
        this.form.leafCode=value[value.length-1];
        this.getName(this.options); //拿到名字,目前只得到最后一级的名称
      },
      getName(arr){
      	for(let item of arr){
      		if(item.value==this.form.leafCode){
      			this.form.leafName=item.label;
      			break;
      		}
      		if(item.children){
    ...
    mounted() {
      this.selectedOptions = this.form.areaCode;
    }
  }
</script>
