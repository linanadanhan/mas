<template>
	<div>
	    ...
	    <el-collapse-item name="2" title="高级设置">
	      ...
	      <el-form-item v-show="context.column.dataSource === '1'" label="备选项">
  ...
  export default {
    ...
    methods:{
      addList(){
        ...
        let options = this.context.column.options.push({value:String(length+1),label:''})
        this.$set(this.context,this.context.column.options,options)*/
      },
      deleteList(index){
        this.context.column.options.splice(index,1)
      },
      handleDicChange() { // 数据字典下拉选框change事件
