	...
	<div>
	    ...
	    <el-collapse-item name="2" title="高级设置">
	      ...
	      <el-form-item v-show="context.column.dataSource === '3' && context.column.viewKey && context.column.viewKey!='' " label="选择值">
	       	<el-select v-model="context.column.viewValue" @change="handleValueChange" placeholder="请选择值" size="mini">
	      ...
	      <el-form-item label="设置">
	        <el-checkbox v-model="context.column.disabled">禁用</el-checkbox>
	      </el-form-item>
	    </el-collapse-item>
	    <el-collapse-item name="3" title="校验">
	      <el-form-item label="">
	        <el-checkbox v-model="context.column.rule[0].required">必填</el-checkbox><br/>
	      </el-form-item>
	    </el-collapse-item>
	  </el-form>
...
<script>
  ...
  export default {
  	props:['page', 'params', 'widget','context','config','model'],
  	components: {
      'set-item': SetItem
    },
    data() {
    ...
    methods:{
      ...
      handleRadioChange(val) {
      	this.model[this.context.attrName] = [] //清空选中项
