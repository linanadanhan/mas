<template>
	<div class="widget">
		<el-form ref="form" :model="form" label-width="140px">
			<el-form-item prop="formFlag" label="table模版">
...
<script>
	export default {
		name: 'table-browser-widget-config-a',
		title: '系统设置',
		props: ['data', 'form'],
		data() {
			return {
		...
		methods: {
			...
			getFormFlagOptions() { // 获取表单标识下拉选框值
					...
					if(res && res.id) {
						...
						if(tmpConfig.table.resource) {
							this.$api_form_getTableFeild({
								data: {
									tableName: tmpConfig.table.resource,
									dataSourceType: tmpConfig.table.dataSourceType
								}
							}).then((data) => {
								if(data) {
									this.feildList = data
