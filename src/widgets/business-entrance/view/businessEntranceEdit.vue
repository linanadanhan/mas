			  ...
			  <el-checkbox-group style="font-size: 14px;" v-model="checkedSys" @change="handleCheckedSysChange">
			  	<div v-for="sysAccess in sysAccessOpts" style="float:left">
			  		<el-row :gutter="2" style="width: 180px;">
			  			<el-col :span="3" >
			  				<el-checkbox :label="sysAccess.C_YW_CODE" :key="sysAccess.C_YW_CODE"><p></p></el-checkbox>
			  			</el-col>
			  			<el-col :span="8">
			  				<img class="avatar" :src="filterIco(sysAccess.C_YW_ICO)" v-if="sysAccess.C_YW_ICO">
							<img class="avatar" v-else src="../noImg.jpg" />  
			  			</el-col>
			  			<el-col :span="13">
			  				<div>
								<p style="width: 80px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" :title="sysAccess.C_YW_NAME">{{sysAccess.C_YW_NAME}}</p>
		  ...
		  <div class="text item" style="padding-bottom: 35px;">
			  ...
			  <el-checkbox-group style="font-size: 14px;" v-model="checkedCus" @change="handleCheckedCusChange">
			    <div v-for="cusAccess in cusAccessOpts" style="float:left">
			  		<el-row :gutter="2" style="width: 180px;">
			  			<el-col :span="3" >
			  				<el-checkbox :label="cusAccess.C_YW_CODE" :key="cusAccess.C_YW_CODE"><p></p></el-checkbox>
			  			</el-col>
			  			<el-col :span="8">
			  				<img class="avatar" :src="filterIco(cusAccess.C_YW_ICO)" v-if="cusAccess.C_YW_ICO">
							<img class="avatar" v-else src="../noImg.jpg" />  
			  			</el-col>
			  			<el-col :span="13">
			  				<div>
								<p style="width: 80px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" :title="cusAccess.C_YW_NAME">{{cusAccess.C_YW_NAME}}</p>
		...
		methods: {
			loadData(params) {
				...
				this.dataList.forEach((item, index) => {
					if (item.C_TYPE == '0') {
						this.checkedSys.push(item.C_YW_CODE)
					}
					
					if (item.C_TYPE == '1') {
						this.checkedCus.push(item.C_YW_CODE)
					}
				})
			...
			getAllSysAccessList() {
				let paraMap = {
				  	tableName: 'COS_SYS_BUSINESS_ENTRANCE'
				}
			...
			getAllCusAccessList() {
				let paraMap = {
				  	tableName: 'COS_CUSTOM_BUSINESS_ENTRANCE',
			...
			saveAddApply() {
				...
				if (chkArr.length > 0) {
					...
					console.info(chkArr)
					this.$api_form_batchSaveBusinessEntrance(options).then(res => {
						this.$message({
							'message': '保存成功！',
							'type': 'success'
						});
...
//				let form = item
//				form.tableName = 'COS_CUSTOM_BUSINESS_ENTRANCE'
...
//				let form = item
//				form.tableName = 'COS_SYS_BUSINESS_ENTRANCE'
//				this.$api_form_saveFormData({
//					data: form
//				}).then(res => {
//					console.info(res)
