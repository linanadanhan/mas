<template>
	<decorator :i="i" :j="j" :widget="widget" :page="page">
					...
					<el-card shadow="hover" class="business-card">
						<el-row type="flex">
							<div class="business-avatar">
								<img v-if="item.C_SYS_ICO" :src="item.sysIcoRef" class="avatar">
								<img v-else src="./noImg.jpg" class="avatar" />
							</div>
							<div class="business-info">
								<h5 class="business-title" :title="item.C_SYS_NAME">{{item.C_SYS_NAME}}</h5>
		...
		<el-dialog size="small" title="" :visible.sync="editDialog" :append-to-body="true" :close-on-click-modal="false" width="100%" top="6vh">
			<businessAccessEdit @closeAndFresh="closeAndFresh" :data="page" :widgetUuId="widget.id" :dataList="dataList" ref="eidtDialogInfo"></businessAccessEdit>
		</el-dialog>
...
<script>
	...
	import businessAccessEdit from './view/businessAccessEdit.vue'
	export default {
		name: 'bar-widget',
		...
		components: {
			'businessAccessEdit': businessAccessEdit
		},
		computed: {
			editMode() {
		...
		methods: {
			handleClick(item) {
				if(item.C_ACCESS_URL) {
					window.open(item.C_ACCESS_URL, '_blank')
				}
			},
			getDataList() {
				...
				this.$api_form_getTableDataInfo({
					...
					if(res1) {
						res1.forEach((item, index) => {
							if(item.C_SYS_ICO) {
								item.sysIcoRef = this.fileServiceUrl + item.C_SYS_ICO
							}
						})
						this.dataList = res1
					}
